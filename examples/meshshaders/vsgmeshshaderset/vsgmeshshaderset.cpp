...
vsg::ref_ptr<vsg::Node> custom::MeshGltfBuilder::createMesh(vsg::ref_ptr<vsgXchange::gltf::Mesh> gltf_mesh, const vsgXchange::gltf::Builder::MeshExtras& meshExtras)
{
    ...
    */
    ...
    for (auto& primitive : gltf_mesh->primitives.values)
    {
        ...
        bool use_meshoptimizer = vsg::value<bool>(false, vsgXchange::gltf::optimize_mesh, options);
        bool build_meshlets = vsg::value<bool>(false, vsgXchange::gltf::build_meshlets, options);
        ...
        if (use_meshoptimizer || build_meshlets)
        {
            ...
            if (optimizeMesh || build_meshlets)
            {
                ...
                {
                    vsg::info("mesh reduction ratio = ", float(optimized_vertexCount)/float(vertexCount));
                    vsg::info("index ratio = ", float(remapped_indices.size())/float(original_indices.size()), ", indices changed = ", vsg::compare_value_container(original_indices, remapped_indices));
                }
                else
                {
